...
# Load data and run for T=0.65 specifically to get all trade dates
lake = MarketDataLake()
...
moe = MoEMetaOrchestrator(confidence_threshold=0.65, gbdt_threshold=0.65, mode="hybrid_v3")
...
for d_str in unique_dates:
    ...
    while b_idx < n_bars:
        ...
        trades.append({
            ...
            "exit_price": exit_px,
            "return_pct": round((pnl_amount / invested) * 100, 2),
            "pnl_krw": int(pnl_amount),
...
print("=== GBDT 65% Monthly Breakdown ===")
monthly_summary = []
for m, grp in df_tr.groupby('month'):
    tot = len(grp)
    wins = len(grp[grp['is_win']])
    losses = len(grp[~grp['is_win']])
    wr = (wins / tot * 100) if tot > 0 else 0
    pnl = grp['pnl_krw'].sum()
    end_cap = grp['capital_after'].iloc[-1]
    soxl_cnt = len(grp[grp['symbol'] == 'SOXL'])
    soxs_cnt = len(grp[grp['symbol'] == 'SOXS'])
...
print("\nTotal Trades:", len(df_tr))
print(f"Weekly Average: {len(df_tr) / total_weeks:.2f} trades / week")
print(f"Monthly Average: {len(df_tr) / num_months:.2f} trades / month")
