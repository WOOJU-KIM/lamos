...
def run_real_backtest():
    ...
    tqqq = pd.read_sql("SELECT datetime, open as Open, high as High, low as Low, close as Close, volume as Volume FROM market_candles WHERE symbol=config.TICKER_LONG ORDER BY datetime", conn)
    sqqq = pd.read_sql("SELECT datetime, open as Open, high as High, low as Low, close as Close, volume as Volume FROM market_candles WHERE symbol=config.TICKER_SHORT ORDER BY datetime", conn)
    vix = pd.read_sql("SELECT datetime, close as VIX FROM market_candles WHERE symbol=config.MACRO_TICKER_3 ORDER BY datetime", conn)
    tnx = pd.read_sql("SELECT datetime, close as TNX FROM market_candles WHERE symbol='^TNX' ORDER BY datetime", conn)
    nvda = pd.read_sql("SELECT datetime, close as NVDA_Close FROM market_candles WHERE symbol=config.MACRO_TICKER_2 ORDER BY datetime", conn)
    qqq = pd.read_sql("SELECT datetime, close as QQQ_Close FROM market_candles WHERE symbol=config.MACRO_TICKER_1 ORDER BY datetime", conn)
    vixy = pd.read_sql("SELECT datetime, close as VIXY_Close FROM market_candles WHERE symbol=config.MACRO_TICKER_3 ORDER BY datetime", conn)
    ief = pd.read_sql("SELECT datetime, close as IEF_Close FROM market_candles WHERE symbol=config.MACRO_TICKER_4 ORDER BY datetime", conn)
    ...
    master_5m = tqqq.copy()
    master_5m = pd.merge(master_5m, sqqq[['datetime', 'Close']], on='datetime', how='left', suffixes=('', '_short'))
    master_5m = master_5m.rename(columns={'Close_short': 'SQQQ_Close'})
    ...
    long_feat = ml.extract_features(df_15m)
    ...
    for row in df_merged.itertuples():
        ...
        if row.datetime.hour == 15 and row.datetime.minute >= 50 and pos > 0:
            if current_sym == config.TICKER_LONG: last_px = row.Close
        ...
        if pos == 0:
            ...
            if current_sym == config.TICKER_LONG: cur_px = row.Close
    ...
    if pos > 0:
        if current_sym == config.TICKER_LONG: last_px = df_merged.iloc[-1]['Close']
    ...
    test_start = long_feat_pred['datetime'].min()
    test_end = long_feat_pred['datetime'].max()
