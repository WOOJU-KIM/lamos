...
long_15m = pd.read_sql_query("SELECT datetime, open as Open, high as High, low as Low, close as Close, volume as Volume FROM market_candles WHERE symbol=config.TICKER_LONG AND timeframe='15m' ORDER BY datetime", conn)
short_15m = pd.read_sql_query("SELECT datetime, open as Open, high as High, low as Low, close as Close, volume as Volume FROM market_candles WHERE symbol=config.TICKER_SHORT AND timeframe='15m' ORDER BY datetime", conn)
nvda_15m = pd.read_sql_query("SELECT datetime, open as Open, high as High, low as Low, close as Close, volume as Volume FROM market_candles WHERE symbol=config.MACRO_TICKER_2 AND timeframe='15m' ORDER BY datetime", conn)
qqq_15m = pd.read_sql_query("SELECT datetime, open as Open, high as High, low as Low, close as Close, volume as Volume FROM market_candles WHERE symbol=config.MACRO_TICKER_1 AND timeframe='15m' ORDER BY datetime", conn)
trend_15m = pd.read_sql_query("SELECT datetime, open as Open, high as High, low as Low, close as Close, volume as Volume FROM market_candles WHERE symbol=config.TICKER_TREND AND timeframe='15m' ORDER BY datetime", conn)
vix_15m = pd.read_sql_query("SELECT datetime, open as Open, high as High, low as Low, close as Close, volume as Volume FROM market_candles WHERE symbol=config.MACRO_TICKER_3 AND timeframe='15m' ORDER BY datetime", conn)
...
long_60m = pd.read_sql_query("SELECT datetime, open as Open, high as High, low as Low, close as Close, volume as Volume FROM market_candles WHERE symbol=config.TICKER_LONG AND timeframe='60m' ORDER BY datetime", conn)
trend_60m = pd.read_sql_query("SELECT datetime, open as Open, high as High, low as Low, close as Close, volume as Volume FROM market_candles WHERE symbol=config.TICKER_TREND AND timeframe='60m' ORDER BY datetime", conn)
conn.close()

for df in [long_15m, short_15m, nvda_15m, qqq_15m, trend_15m, vix_15m, tnx_15m, long_60m, trend_60m]:
...
ema12 = long_60m['Close'].ewm(span=12, adjust=False).mean()
ema26 = long_60m['Close'].ewm(span=26, adjust=False).mean()
long_60m['macd'] = ema12 - ema26
long_60m['macd_sig'] = long_60m['macd'].ewm(span=9, adjust=False).mean()
...
unique_dates = sorted(long_15m['date'].unique())
total_weeks = len(unique_dates) / 5.0

print(f"Loaded {len(long_15m)} 15m candles over {len(unique_dates)} trading days ({total_weeks:.1f} weeks).")
...
class PrecisionMoESigmoidNetwork:
    ...
    def evaluate_bar(
        ...
        trend_sub: pd.DataFrame,
        tnx_sub: pd.DataFrame,
        trend_60_sub: pd.DataFrame,
        long_60_sub: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        s_ret = (long_sub['Close'].iloc[-1] / long_sub['Close'].iloc[-5] - 1.0) * 100.0 if len(long_sub) >= 5 else 0.0
        ...
        if top_expert == "orderflow":
            cvd_df = exp_orderflow.compute_cvd(long_sub)
            s_code, c_val, _ = exp_orderflow.predict_signal(cvd_df.iloc[-1])
            if s_code != 0:
                expert_sig = s_code
                direction = f"LONG_{config.TICKER_LONG}" if s_code == 1 else f"SHORT_{config.TICKER_SHORT}"
                expert_conf = max(top_gating_conf, c_val)

        elif top_expert == "tda_topology":
            s_code, c_val, _ = exp_tda.predict_signal(long_sub)
            if s_code != 0:
                expert_sig = s_code
                direction = f"LONG_{config.TICKER_LONG}" if s_code == 1 else f"SHORT_{config.TICKER_SHORT}"
                expert_conf = max(top_gating_conf, c_val)
            ...
            s_code, c_val, _ = exp_statespace.predict_signal(long_sub['Close'].values)
            if s_code != 0:
                expert_sig = s_code
                direction = f"LONG_{config.TICKER_LONG}" if s_code == 1 else f"SHORT_{config.TICKER_SHORT}"
                expert_conf = max(top_gating_conf, c_val)
            ...
            sx_ret = (trend_sub['Close'].iloc[-1] / trend_sub['Close'].iloc[-5] - 1.0) if len(trend_sub) >= 5 else 0.0
            ...
            is_60m_bull = False
            is_60m_bear = False
            if len(trend_60_sub) >= 20 and len(long_60_sub) >= 20:
                last_trend = trend_60_sub.iloc[-1]
                last_long = long_60_sub.iloc[-1]
                is_60m_bull = (last_trend['Close'] >= last_trend['ema20'] * 0.998) and (last_long['Close'] >= last_long['ema20'] * 0.998)
                is_60m_bear = (last_trend['Close'] <= last_trend['ema20'] * 1.002) and (last_long['Close'] <= last_long['ema20'] * 1.002)

            long_dip_ok = (row_l.get('VWAP_Diff', 0) <= 1.5) and (row_l.get('RSI_14', 50) <= 62.0)
            short_dip_ok = (row_s.get('VWAP_Diff', 0) <= 1.5) and (row_s.get('RSI_14', 50) <= 62.0)

            if is_60m_bull and long_dip_ok:
                expert_sig = 1
                direction = f"LONG_{config.TICKER_LONG}"
            elif is_60m_bear and short_dip_ok:
                expert_sig = -1
...
def run_precision_sweep():
    ...
    for T in THRESHOLDS:
        ...
        for day_str in unique_dates:
            day_long = long_15m[long_15m['date'] == day_str].reset_index(drop=True)
            day_short = short_15m[short_15m['date'] == day_str].reset_index(drop=True)
            ...
            for b_idx in range(num_bars):
                row_l = long_feat[long_feat['date'] == day_str].iloc[b_idx]
                row_s = short_feat[short_feat['date'] == day_str].iloc[b_idx]
                ...
                long_sub = long_15m[long_15m['datetime'] <= bar_dt].tail(30)
                nvda_sub = nvda_15m[nvda_15m['datetime'] <= bar_dt].tail(30)
                qqq_sub = qqq_15m[qqq_15m['datetime'] <= bar_dt].tail(30)
                trend_sub = trend_15m[trend_15m['datetime'] <= bar_dt].tail(30)
                tnx_sub = tnx_15m[tnx_15m['datetime'] <= bar_dt].tail(30)

                trend_60_sub = trend_60m[trend_60m.index <= row_l.name]
                long_60_sub = long_60m[long_60m.index <= row_l.name]
                ...
                if not in_market and current_pos == "NONE":
                    ...
                    eval_res = orchestrator.evaluate_bar(
                        ...
                        long_sub, nvda_sub, qqq_sub, trend_sub, tnx_sub,
                        trend_60_sub, long_60_sub
                    )
                    ...
                    if eval_res["gating_confidence"] >= T and eval_res["expert_sig"] != 0 and eval_res["direction"] != "NONE":
                        in_market = True
                        current_pos = config.TICKER_LONG if eval_res["direction"] == f"LONG_{config.TICKER_LONG}" else config.TICKER_SHORT
