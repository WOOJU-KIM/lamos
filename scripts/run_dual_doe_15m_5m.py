...
def run_comprehensive_dual_doe():
    ...
    for df in [tqqq_15m, sqqq_15m, tqqq_5m, sqqq_5m, soxx_60m, tqqq_60m, soxx_15m, soxx_5m, nvda_15m, nvda_5m, qqq_15m, qqq_5m, vix_15m, vix_5m]:
        if df.empty:
            continue
    ...
    print(f"   • 분석 기간: {tqqq_15m['datetime'].iloc[0]} ~ {tqqq_15m['datetime'].iloc[-1]}")
    ...
    feat_5m_tqqq = sniper.ml_engine.extract_features(tqqq_5m)
    X_5m_vec = feat_5m_tqqq[sniper.feature_cols].fillna(0.0)
    probs_5m = sniper.model.predict_proba(X_5m_vec) # [p_short, p_flat, p_long]
    feat_5m_tqqq['p_short'] = probs_5m[:, 0]
    feat_5m_tqqq['p_flat'] = probs_5m[:, 1]
    feat_5m_tqqq['p_long'] = probs_5m[:, 2]
    ...
    tqqq_15m_close = tqqq_15m['Close'].values
    tqqq_15m_dt = tqqq_15m['datetime'].values
    ...
    for i in range(len(tqqq_15m)):
        if i < 5:
            ...
            continue
        cur_t = tqqq_15m_dt[i]
        past_5_t = tqqq_15m_dt[i-5]
        s_r = float(tqqq_15m_close[i] / tqqq_15m_close[i-5] - 1.0)
        if (cur_t in nvda_15m_map and past_5_t in nvda_15m_map and
            ...
            sig_code, _, _ = cross_mod.predict_signal(
                tqqq_ret=s_r, nvda_ret=n_r, soxx_ret=sx_r, qqq_ret=q_r, vix_ret=v_r, tnx_ret=0.0
            )
            c_dir = "LONG_TQQQ" if sig_code > 0 else ("SHORT_SQQQ" if sig_code < 0 else "NONE")
        else:
            c_dir = "NONE"
        cross_dirs_15m.append(c_dir)
    tqqq_15m_feat['cross_dir'] = cross_dirs_15m
    ...
    tqqq_5m_close = tqqq_5m['Close'].values
    tqqq_5m_dt = tqqq_5m['datetime'].values

    cross_dirs_5m = []
    for i in range(len(tqqq_5m)):
        if i < 5:
            cross_dirs_5m.append("HOLD")
            continue
        cur_t = tqqq_5m_dt[i]
        past_5_t = tqqq_5m_dt[i-5]
        s_r = float(tqqq_5m_close[i] / tqqq_5m_close[i-5] - 1.0)
        if (cur_t in nvda_5m_map and past_5_t in nvda_5m_map and
            ...
            c_dir = "LONG_TQQQ" if sig_code > 0 else ("SHORT_SQQQ" if sig_code < 0 else "HOLD")
        else:
            c_dir = "HOLD"
        cross_dirs_5m.append(c_dir)
    feat_5m_tqqq['cross_dir'] = cross_dirs_5m
    feat_5m_tqqq.set_index('datetime', inplace=True, drop=False)
    ...
    for T in doe_thresholds:
        ...
        for d_str in unique_dates:
            ...
            while b_idx < n_bars:
                ...
                if len(past_soxx_60) >= 20 and len(past_tqqq_60) >= 20:
                    ...
                    tqqq_ema = past_tqqq_60['ema20'].iloc[-1]
                    if direction == "LONG_TQQQ":
                        is_60m_ok = (soxx_c >= soxx_ema * 0.998) and (tqqq_c >= tqqq_ema * 0.998)
                ...
                chosen_sym = "TQQQ" if direction == "LONG_TQQQ" else "SQQQ"
                base_px = float(row_15['Close']) if chosen_sym == "TQQQ" else (float(sqqq_15m_feat.loc[cur_time]['Close']) if cur_time in sqqq_15m_feat.index else 40.0)
        ...
        tqqq_cnt = len([t for t in trades if t['symbol'] == 'TQQQ'])
        tqqq_w = len([t for t in trades if t['symbol'] == 'TQQQ' and t['ret'] > 0])
        tqqq_wr = (tqqq_w / tqqq_cnt * 100) if tqqq_cnt > 0 else 0.0

        sqqq_cnt = len([t for t in trades if t['symbol'] == 'SQQQ'])
        sqqq_w = len([t for t in trades if t['symbol'] == 'SQQQ' and t['ret'] > 0])
        sqqq_wr = (sqqq_w / sqqq_cnt * 100) if sqqq_cnt > 0 else 0.0
        ...
        results_15m.append({
            ...
            "losses": len(l_tr),
            "win_rate": wr,
            "tqqq_trades": tqqq_cnt,
            "tqqq_wr": tqqq_wr,
            "sqqq_trades": sqqq_cnt,
            "sqqq_wr": sqqq_wr,
            "cum_return": cum_ret,
            "profit_factor": pf,
            "mdd": mdd,
    ...
    for T in doe_thresholds:
        ...
        for d_str in unique_dates:
            day_5_l = feat_5m_tqqq[feat_5m_tqqq['date_str'] == d_str]
            day_5_s = sqqq_5m[sqqq_5m['date_str'] == d_str]
            if len(day_5_l) < 20:
                continue
            ...
            while idx < n_bars:
                ...
                if direction == "LONG_TQQQ" and (last_soxx['Close'] < last_soxx['ema20'] * 0.998):
                    idx += 1
                    continue
                elif direction == "SHORT_SQQQ" and (last_soxx['Close'] > last_soxx['ema20'] * 1.002):
                    idx += 1
                    continue
                ...
                chosen_sym = "TQQQ" if direction == "LONG_TQQQ" else "SQQQ"
                base_px = float(row_5['Close']) if chosen_sym == "TQQQ" else float(day_5_s[day_5_s['datetime'] == cur_t_str]['Close'].iloc[0]) if not day_5_s[day_5_s['datetime'] == cur_t_str].empty else 40.0
                entry_px = round(base_px + SLIPPAGE, 2)
                shares = int(capital / entry_px)
                invested = shares * entry_px
                if shares <= 0:
                ...
                target_day = day_5_l if chosen_sym == "TQQQ" else day_5_s
...
if __name__ == "__main__":
    ...
    headers = ["확신도", "총거래", "주당거래", "승 / 패", "승률(%)", "TQQQ(건/승률)", "SQQQ(건/승률)", "누적수익률", "손익비(PF)", "MDD(%)", "최종자본금($)"]
    aligns = [":---:", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:", "---:", ":---:", ":---:", "---:"]
    ...
    for m in r15:
        rows_15.append([
            ...
            f"{m['tqqq_trades']}건 ({m['tqqq_wr']:.1f}%)", f"{m['sqqq_trades']}건 ({m['sqqq_wr']:.1f}%)",
