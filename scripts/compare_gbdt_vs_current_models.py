...
def run_comparison():
    ...
    for i in range(len(long_15m)):
        ...
        if c_t in nvda_map and p_t in nvda_map and c_t in qqq_map and p_t in qqq_map and c_t in vix_map and p_t in vix_map:
            ...
            code, conf, desc = cross_mod.predict_signal(long_ret=s_r, nvda_ret=n_r, trend_ret=sx_r, qqq_ret=q_r, vix_ret=v_r, tnx_ret=0.0)
        ...
        for d_str in unique_dates:
            ...
            while b_idx < n_bars:
                ...
                if mode == "gbdt_only":
                    ...
                    if dir_gbdt in [f"LONG_{config.TICKER_LONG}", f"SHORT_{config.TICKER_SHORT}"] and conf_gbdt >= 0.60:
                    ...
                    if dir_gbdt == f"LONG_{config.TICKER_LONG}" and dir_cross == f"LONG_{config.TICKER_LONG}" and conf_gbdt >= 0.60 and conf_cross >= 0.60:
                        chosen_sym = config.TICKER_LONG
                    elif dir_gbdt == f"SHORT_{config.TICKER_SHORT}" and dir_cross == f"SHORT_{config.TICKER_SHORT}" and conf_gbdt >= 0.60 and conf_cross >= 0.60:
                        chosen_sym = config.TICKER_SHORT
                ...
                if chosen_sym == config.TICKER_SHORT and cur_time not in short_15m_feat.index:
                    b_idx += 1
                    continue
                ...
                if use_tp_sl:
                    ...
                    for k15 in range(len(post_15)):
                        ...
                        d15 = r15['Direction']
                        # if signal reverses
                        opp_sig = f"SHORT_{config.TICKER_SHORT}" if chosen_sym == config.TICKER_LONG else f"LONG_{config.TICKER_LONG}"
                        if d15 == opp_sig or t15 >= '15:50':
                            if chosen_sym == config.TICKER_LONG:
                                c15_close = float(r15['Close'])
                            else:
                                if r15['datetime'] in short_15m_feat.index:
                                    c15_close = float(short_15m_feat.loc[r15['datetime']]['Close'])
                                else:
                                    c15_close = float(short_15m_feat.iloc[-1]['Close'])
                            exit_px = round(c15_close - SLIPPAGE, 2)
                            exit_reason = "신호 반전 청산" if d15 == opp_sig else "EOD (15:50)"
                            bars_held = (k15 + 1) * 3
                            break
                    else:
                        c_last = float(day_15.iloc[-1]['Close']) if chosen_sym == config.TICKER_LONG else float(short_15m_feat.iloc[-1]['Close'])
