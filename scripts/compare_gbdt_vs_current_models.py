        ...
        for d_str in unique_dates:
            ...
            while b_idx < n_bars:
                ...
                if mode == "gbdt_only":
                    ...
                    if dir_gbdt in ['LONG_TQQQ', 'SHORT_SQQQ'] and conf_gbdt >= 0.60:
                    ...
                    if dir_gbdt == 'LONG_TQQQ' and dir_cross == 'LONG_TQQQ' and conf_gbdt >= 0.60 and conf_cross >= 0.60:
                        chosen_sym = 'TQQQ'
                    elif dir_gbdt == 'SHORT_SQQQ' and dir_cross == 'SHORT_SQQQ' and conf_gbdt >= 0.60 and conf_cross >= 0.60:
                        chosen_sym = 'SQQQ'
                ...
                if chosen_sym == 'SQQQ' and cur_time not in sqqq_15m_feat.index:
                    b_idx += 1
                    continue
                ...
                if use_tp_sl:
                    ...
                    for k15 in range(len(post_15)):
                        ...
                        d15 = r15['Direction']
                        # if signal reverses
                        opp_sig = 'SHORT_SQQQ' if chosen_sym == 'TQQQ' else 'LONG_TQQQ'
                        if d15 == opp_sig or t15 >= '15:50':
                            if chosen_sym == 'TQQQ':
                                c15_close = float(r15['Close'])
                            else:
                                if r15['datetime'] in sqqq_15m_feat.index:
                                    c15_close = float(sqqq_15m_feat.loc[r15['datetime']]['Close'])
                                else:
                                    c15_close = float(sqqq_15m_feat.iloc[-1]['Close'])
                            exit_px = round(c15_close - SLIPPAGE, 2)
                            exit_reason = "신호 반전 청산" if d15 == opp_sig else "EOD (15:50)"
                            bars_held = (k15 + 1) * 3
                            break
                    else:
                        c_last = float(day_15.iloc[-1]['Close']) if chosen_sym == 'TQQQ' else float(sqqq_15m_feat.iloc[-1]['Close'])
