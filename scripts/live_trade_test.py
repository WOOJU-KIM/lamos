...
#         - '-2.0% ?? ?  ??daily_stoploss_count 1 ? (?/?????)
        ...
        if self.daily_stoploss_count >= 3:
            ...
            try:
                pass; # self.dispatcher.send_telegram_message(msg)
    ...
    def _execute_buy_with_10s_chase(
        ...
        try:
            ...
            pass; # self.dispatcher.send_telegram_message(buy_msg)
        ...
        try:
            ...
            while True:
                ...
                if is_sold:
                    ...
                    pass; # self.dispatcher.send_telegram_message(sell_msg)
                    if is_stoploss:
...
• <b>VIXY</b>: {live_prices.get(config.MACRO_TICKER_3, 0.0):.2f} / <b>IEF</b>: {live_prices.get(config.MACRO_TICKER_4, 0.0):.2f}
• <b>상태</b>: {'진입 승인 🚀' if is_appr else '관망 중 👁️'}
"""
                            pass; # self.dispatcher.send_telegram_message(briefing_msg)
    ...
    def run_once_and_start_listener(self):
        ...
        # 3. ?  ? ??????
        ai_engine_desc = f"`Phase 1: 15 GBDT Model ({config.GBDT_CONFIDENCE_THRESHOLD*100:.0f}% ? ??/ ????Veto ?? / GBDT Feature ?)`"
...
??, ???????? ???? ???."""
        try:
            pass; # self.dispatcher.send_telegram_message(start_msg)
