...
def run_live_identical_backtest():
    ...
    for s in symbols:
        df = pd.read_sql_query(
            f"SELECT datetime, open, high, low, close, volume FROM market_candles WHERE symbol='{s}' AND timeframe='15m' ORDER BY datetime ASC",
            conn
        )
    ...
    CONFIDENCE_THRESHOLD = 0.75   # 75.0% 절대확신도 게이팅 기준
