...
for i in range(50, len(tqqq_df)):
    sub_tqqq = tqqq_df.iloc[:i]
    ...
    # M2
    cvd_df = m2.compute_cvd(sub_tqqq.tail(30))
    s2, c2, _ = m2.predict_signal(cvd_df.iloc[-1])
    if s2 != 0:
        sig_m2 += 1
        
    # M3
    s3, c3, _ = m3.predict_signal(sub_tqqq.tail(30))
    if s3 != 0:
        sig_m3 += 1
        
    # M4
    s4, c4, _ = m4.predict_signal(sub_tqqq['Close'].tail(30).values)
    if s4 != 0:
        sig_m4 += 1
        
    # M5
    s_ret = (sub_tqqq['Close'].iloc[-1] / sub_tqqq['Close'].iloc[-5] - 1.0)
...
print(f"Total bars: {len(tqqq_df)-50}")
