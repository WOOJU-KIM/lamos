...
print("=" * 80)
print("🔍 [SOXL 783주 매수 체결 원인 정밀 포렌식 분석]")
print("=" * 80)
...
if logs_file.exists():
    ...
    print("• 매수/주문/SOXL 관련 로그 목록:")
    for l in lines:
        msg = l.get("message", "")
        src = l.get("source", "")
        lvl = l.get("level", "")
        dt = l.get("datetime", "")
        if any(w in msg.lower() for w in ["soxl", "매수", "autoexecution", "783", "124.5", "buy"]):
...
cur = conn.cursor()
rows = cur.execute("SELECT * FROM shadow_trades WHERE ticker = 'SOXL' ORDER BY rowid DESC LIMIT 10;").fetchall()
print(f"\n• Shadow Trades SOXL 최근 기록 ({len(rows)}건):")
for r in rows:
