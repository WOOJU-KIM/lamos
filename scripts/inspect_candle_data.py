...
df_soxl = pd.read_sql_query("SELECT datetime, open, high, low, close, volume FROM market_candles WHERE symbol='SOXL' AND timeframe='15m' ORDER BY datetime", conn)
print('SOXL 15m rows:', len(df_soxl))
df_soxl['date'] = df_soxl['datetime'].str.slice(0, 10)
unique_dates = df_soxl['date'].unique()
