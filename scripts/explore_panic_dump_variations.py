...
def explore_variations():
    ...
    df_l = pd.read_sql_query("SELECT datetime, open, high, low, close, volume FROM market_candles WHERE symbol=config.TICKER_LONG AND timeframe='5m' ORDER BY datetime ASC", conn)
    df_s = pd.read_sql_query("SELECT datetime, open, high, low, close, volume FROM market_candles WHERE symbol=config.TICKER_SHORT AND timeframe='5m' ORDER BY datetime ASC", conn)
