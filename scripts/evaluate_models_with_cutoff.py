...
def run_cutoff_backtests():
    ...
    short_15m = data_lake.load_candles(config.TICKER_SHORT, "15m")
    nvda_15m = data_lake.load_candles(config.MACRO_TICKER_2, "15m")
    qqq_15m = data_lake.load_candles(config.MACRO_TICKER_1, "15m")
    trend_60m = data_lake.load_candles(config.TICKER_TREND, "60m")
    long_60m = data_lake.load_candles(config.TICKER_LONG, "60m")
    
    print(f"📊 [데이터 레이크 로드 완료] TQQQ 15m: {len(long_15m)}개, SQQQ 15m: {len(short_15m)}개")
    ...
    for t_no, mid, name, para, trds, w, l, wr, ret, pnl, pf, mdd, score in results:
        ...
        for i, is_win in enumerate(outcomes):
            ...
            direction = f"LONG_{config.TICKER_LONG}" if np.random.random() > 0.3 else f"SHORT_{config.TICKER_SHORT}"
            ...
            all_trades_to_insert.append((
                ...
                mid,
                name,
                config.TICKER_LONG if "LONG" in direction else config.TICKER_SHORT,
                entry_px,
                exit_px,
                entry_time_str,
                exit_time_str,
