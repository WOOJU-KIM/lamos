...
print(f"\n[1/4] SOXL 실시간 호가 및 목표가 산출:")
print(f"   • SOXL 현재가: ${cur_px:.2f}")
print(f"   • 목표 익절가 (+3.5% 지정가 예약매도): ${tp_px:.2f}")
print(f"   • 칼손절 기준가 (-2.0%): ${sl_px:.2f}")

# 2. 실시간 시장가 매수 발주 시험
print(f"\n[2/4] 키움 OpenAPI ➔ SOXL {qty}주 실시간 시장가 매수 발주...")
buy_res = broker.send_order(symbol="SOXL", order_type="BUY", quantity=qty, price=0.0)
print(f"   • 매수 주문 응답: ok={buy_res.get('ok')}, 주문번호={buy_res.get('order_no')}, 메시지={buy_res.get('msg')}")

# 3. 매수 직후 +3.5% 지정가 예약매도 발주 시험
print(f"\n[3/4] 키움 OpenAPI ➔ SOXL {qty}주 @ ${tp_px:.2f} (+3.5% 지정가 예약매도) 발주...")
tp_res = broker.send_order(symbol="SOXL", order_type="SELL", quantity=qty, price=tp_px)
