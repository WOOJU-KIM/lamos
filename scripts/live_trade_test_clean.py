    ...
    while True:
        ...
        if current_time.minute % 15 == 0:
            if last_eval_time != current_time:
                ...
                df_15m = data_lake.load_candles(config.TICKER_LONG, "15m")
                if df_15m.empty:
                    continue
                    
                live_prices = {
                    config.TICKER_LONG: feeder.get_latest_price(config.TICKER_LONG, 0.0),
                    config.TICKER_SHORT: feeder.get_latest_price(config.TICKER_SHORT, 0.0),
                    config.TICKER_TREND: feeder.get_latest_price(config.TICKER_TREND, 0.0),
                    config.MACRO_TICKER_1:  feeder.get_latest_price(config.MACRO_TICKER_1, 0.0),
                    config.MACRO_TICKER_2: feeder.get_latest_price(config.MACRO_TICKER_2, 0.0),
                    config.MACRO_TICKER_3: feeder.get_latest_price(config.MACRO_TICKER_3, 0.0),
                    config.MACRO_TICKER_4:  feeder.get_latest_price(config.MACRO_TICKER_4, 0.0),
                }
