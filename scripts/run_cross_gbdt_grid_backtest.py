...
def run_grid_search():
    ...
    for i in range(len(tqqq_15m)):
        ...
        if c_t in nvda_map and p_t in nvda_map and c_t in qqq_map and p_t in qqq_map and c_t in vix_map and p_t in vix_map:
            ...
            code, conf, desc = cross_mod.predict_signal(tqqq_ret=s_r, nvda_ret=n_r, soxx_ret=sx_r, qqq_ret=q_r, vix_ret=v_r, tnx_ret=0.0)
    ...
    def run_sim(g_th: float, c_th: float, mode: str = "consensus"):
        ...
        for d_str in unique_dates:
            ...
            while b_idx < n_bars:
                ...
                if mode == "consensus":
                    ...
                    if dir_gbdt == 'LONG_TQQQ' and dir_cross == 'LONG_TQQQ' and conf_gbdt >= g_th and conf_cross >= c_th:
                        chosen_sym = 'TQQQ'
                    elif dir_gbdt == 'SHORT_SQQQ' and dir_cross == 'SHORT_SQQQ' and conf_gbdt >= g_th and conf_cross >= c_th:
                        chosen_sym = 'SQQQ'
                elif mode == "hybrid_gated":
                    # [하이브리드 MoE 점수 게이팅] GBDT>=G, 크로스에셋 반대 Veto 차단 (반대 확신도>=C일 때 Veto), 동방향 시 확인
                    is_gbdt = (dir_gbdt in ['LONG_TQQQ', 'SHORT_SQQQ']) and (conf_gbdt >= g_th)
                    is_opposite_veto = (
                        (dir_gbdt == 'LONG_TQQQ' and dir_cross == 'SHORT_SQQQ' and conf_cross >= c_th) or
                        (dir_gbdt == 'SHORT_SQQQ' and dir_cross == 'LONG_TQQQ' and conf_cross >= c_th)
                    )
        ...
        tqqq_tr = [t for t in trades if t['symbol'] == 'TQQQ']
        sqqq_tr = [t for t in trades if t['symbol'] == 'SQQQ']
        tqqq_w = [t for t in tqqq_tr if t['ret'] > 0]
        sqqq_w = [t for t in sqqq_tr if t['ret'] > 0]
        tqqq_wr = (len(tqqq_w) / len(tqqq_tr) * 100) if tqqq_tr else 0.0
        sqqq_wr = (len(sqqq_w) / len(sqqq_tr) * 100) if sqqq_tr else 0.0
        ...
        return {
            ...
            "losses": len(l_tr),
            "win_rate": wr,
            "tqqq_trades": len(tqqq_tr),
            "tqqq_wr": tqqq_wr,
            "sqqq_trades": len(sqqq_tr),
            "sqqq_wr": sqqq_wr,
            "cum_return": cum_ret,
            "profit_factor": pf,
            "mdd": mdd,
...
if __name__ == "__main__":
    ...
    headers_diag = ["GBDT", "크로스에셋", "총거래", "주당거래", "승 / 패", "승률(%)", "TQQQ(승률)", "SQQQ(승률)", "누적수익률", "손익비(PF)", "MDD(%)", "최종자본금($)"]
    aligns_diag = [":---:", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:", ":---:", "---:", ":---:", ":---:", "---:"]
