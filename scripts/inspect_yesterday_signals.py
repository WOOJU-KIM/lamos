...
print("=== SOXL 최근 5개 15분봉 ===")
print(soxl_15m[["Open", "High", "Low", "Close", "Volume"]].tail(5))

c = soxl_15m["Close"]
ret_5 = float((c.iloc[-1] / c.iloc[-5] - 1.0))
print(f"\nSOXL 5봉 수익률: {ret_5*100:.2f}%")
...
df_feat = gbdt.extract_features(soxl_15m)
...
res = moe.evaluate_dual_filter_signal(soxl_15m, threshold=0.75)
print("\n=== 최종 MoE 평가 결과 ===")
