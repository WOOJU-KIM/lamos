...
def run_5m_precision_backtest(model_train_mode: str = "22_24"):
    ...
    print(f"   📊 15분봉: TQQQ {len(long_15m):,}개 | SQQQ {len(short_15m):,}개")
    print(f"   📊 5분봉:  TQQQ {len(long_5m):,}개 | SQQQ {len(short_5m):,}개")
    ...
    labels = ml_engine.compute_triple_barrier_labels(long_feat)
    ...
    import itertools
    param_grid = {
        "tp": [1.012, 1.015, 1.018, 1.021],
        "sl": [0.992, 0.990, 0.988, 0.986],
        "conf": [0.55, 0.58, 0.60, 0.62]
    }
    keys, values = zip(*param_grid.items())
    combinations = [dict(zip(keys, v)) for v in itertools.product(*values)]
    print(f"\n🔥 [Grid Search] 총 {len(combinations)}개 조합 시뮬레이션 시작...")
    for idx, params in enumerate(combinations):
        tp_px_mult = params["tp"]
        sl_px_mult = params["sl"]
        conf_hurdle = params["conf"]
        ...
        for d_str in unique_dates:
            ...
            while b_idx < n_bars:
                ...
                if len(past_trend_60) >= 20:
                    ...
                    trend_60m_bull = (trend_c >= trend_ema20 * 1.002)
                    trend_60m_bear = (trend_c <= trend_ema20 * 0.998)
                ...
                if b_idx >= 5 and n_px and sx_px and q_px:
                    ...
                    long_r = (cur_long_close / day_bars_15.iloc[b_idx - 5]['Close']) - 1.0
                ...
                if dir_gbdt == f"LONG_{config.TICKER_LONG}" and conf_gbdt >= conf_hurdle and trend_60m_bull and cross_dir != f"SHORT_{config.TICKER_SHORT}":
                    entry_approved = True
                    target_sym = config.TICKER_LONG
                    entry_price = cur_long_close
                elif dir_gbdt == f"SHORT_{config.TICKER_SHORT}" and conf_gbdt >= conf_hurdle and trend_60m_bear and cross_dir != f"LONG_{config.TICKER_LONG}":
                    entry_approved = True
                    target_sym = config.TICKER_SHORT
                    entry_price = cur_short_close
                ...
                if entry_approved and entry_price > 0:
                    ...
                    tp_px = entry_price *     tp_px_mult
                    sl_px = entry_price *     sl_px_mult
                    ...
                    # 최대 36개 5분봉(180분) 동안 캔들 단위 정밀 추적
                    max_eval_bars = min(len(post_5m), 36)
                    ...
                    for k in range(max_eval_bars):
                        ...
                        if hit_tp and hit_sl:
                            ...
                            if dist_to_tp < dist_to_sl:
                                exit_price = tp_px
                                exit_reason = "TAKE_PROFIT (5m High)"
                            else:
                                exit_price = sl_px
                                exit_reason = "STOP_LOSS (5m Low)"
                            exit_dt = b_dt
                            holding_5m_bars = k + 1
                            break
                        elif hit_tp:
                        ...
                        # 180분 타임스탑 도달 (36번째 5분봉 종가)
                        if k == 35:
                            exit_price = b_c
                            exit_reason = "TIME_STOP (180m)"
                            exit_dt = b_dt
                            holding_5m_bars = k + 1
                            break
        ...
        res_dict = {"TP": params["tp"], "SL": params["sl"], "Conf": params["conf"], "FinalCap": current_capital, "Trades": len(trades)}
        results.append(res_dict)
        if idx % 10 == 0: print(f"  -> {idx}/{len(combinations)} 완료")
    print("\n🏆 [Top 10 베스트 파라미터]")
    res_df = pd.DataFrame(results).sort_values(by="FinalCap", ascending=False)
    print(res_df.head(10).to_markdown())
    res_df.to_csv("data/doe_korean_market_results.csv", index=False)
    return
