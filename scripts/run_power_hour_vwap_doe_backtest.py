...
def run_power_hour_vwap_sniper_doe():
    ...
    # Calculate Intraday Cumulative VWAP for SOXL on 5m and 15m
        ...
        for d_str in unique_dates:
            ...
            while b_idx < n_bars:
                ...
                if is_p2:
                    ...
                    # VWAP Lock: Spot > VWAP -> ONLY SOXL allowed; Spot < VWAP -> ONLY SOXS allowed
                    if soxl_spot >= current_vwap:
                        if chosen_sym != 'SOXL':
                            # Reject SOXS signal when price is above VWAP
                            b_idx += 1; continue
                    else:
                        if chosen_sym != 'SOXS':
                            # Reject SOXL signal when price is below VWAP
                            b_idx += 1; continue
