...
class MarketRegimeClassifier:
    ...
    국면에 따라 전략 모드(SOXL 롱 가중치, SOXS 숏 가중치, 스위칭 임계값)를 동적으로 스위칭합니다.
    """
    ...
    def classify(self, historical_data: Dict[str, pd.DataFrame] = None) -> Dict[str, Any]:
        ...
        if historical_data is None:
            # yfinance로 장기 데이터 로드 (SOXL, SOXX, ^VIX)
            ...
            soxx_df = historical_data.get("SOXX", soxl_df)
            vix_df = historical_data.get("VIX")

        # 1. 테크니컬 지표 계산
        close_soxl = soxl_df['Close']
        current_soxl = float(close_soxl.iloc[-1])
        ma20 = float(close_soxl.tail(20).mean())
        ma50 = float(close_soxl.tail(50).mean())
        ma200 = float(close_soxl.tail(200).mean()) if len(close_soxl) >= 200 else float(close_soxl.mean())
        ...
        # 20일 실현 변동성 (연율화)
        returns = close_soxl.pct_change().dropna()
        ...
        # 2. 국면 판정 로직
        is_bull_ma = (current_soxl > ma50) and (ma50 > ma200)
        is_bear_ma = (current_soxl < ma50) and (ma50 < ma200)
        ...
        if current_vix >= 25.0 or realized_vol_20d >= 85.0:
            ...
            params = {
                "soxl_weight_mult": 0.5,
                "soxs_weight_mult": 0.8,
            ...
            description = "반도체 지수가 중장기 이평선 위에서 정배열을 형성하며 VIX가 20pt 미만으로 안정적입니다. SOXL 선제 눌림목 매수를 적극 추천합니다."
            params = {
                "soxl_weight_mult": 1.2,
                "soxs_weight_mult": 0.3,
                "dip_buy_threshold_pct": -0.8,
                "stop_loss_pct": -2.0,
                "hedge_switch_threshold_pct": -2.2,
                "target_regime_mode": "AGGRESSIVE_LONG"
            }
        elif is_bear_ma or (current_soxl < ma200 and soxx_20d_perf < -5.0):
            regime = "BEAR_TREND"
            regime_name_kr = "하락 추세 / 약세장 국면 (Bear Trend)"
            description = "중장기 이평선이 역배열이거나 지수 하방 압력이 큽니다. SOXS 헤지 스위칭을 우선시하고 SOXL 진입은 보수적으로 제한합니다."
            params = {
                "soxl_weight_mult": 0.4,
                "soxs_weight_mult": 1.5,
            ...
            params = {
                "soxl_weight_mult": 1.0,
                "soxs_weight_mult": 0.6,
        ...
        return {
            ...
            "metrics": {
                "current_soxl": round(current_soxl, 2),
                "ma20": round(ma20, 2),
                "ma50": round(ma50, 2),
                "ma200": round(ma200, 2),
