...
6. 운용 퀀트 모델 및 매매 헌법 (Hard Rules):
   - 운용 모델: Lumos V3 Hybrid MoE (Phase 1: 15분봉 Model C / Phase 2: 5분봉 스나이퍼)
   - 매수 진입 3대 인터락: GBDT 확신도 60% 이상 공격수 트리거 + 크로스에셋 역풍 방패(정반대 신호 Veto) + 3중 스크린(60m 추세 & 5m 눌림목)
   - 청산 4대 원칙: +3.0% 목표 익절 / -2.0% 칼손절 / 90분 타임스탑 / 15:50 NYT(04:50 KST) EOD 0% 오버나잇 전량 현금화
