...
class MockKiwoomFeeder:
    def __init__(self, broker: Optional[KiwoomBroker] = None, target_date: str = None):
        ...
        if target_date:
            ...
            df_date = pd.read_sql("SELECT date(datetime) as dt FROM market_candles WHERE symbol=config.TICKER_LONG ORDER BY datetime DESC LIMIT 1", conn)
        ...
        self.subscribed_symbols = [config.TICKER_LONG, config.TICKER_SHORT, config.TICKER_TREND, config.MACRO_TICKER_1, config.MACRO_TICKER_2, config.MACRO_TICKER_3, config.MACRO_TICKER_4]
