...
class ShadowRndEngine:
    ...
    CANDIDATE_POOL = [
        ...
        },
        {
            ...
            "take_profit_pct": 3.5,
            "description": "민감한 -0.8% 선제진입, -1.5% 타이트 손절, 빠른 SOXS 헤지 전환"
        },
        {
        ...
        },
        {
            ...
            "take_profit_pct": 3.0,
            "description": "극단적 하방 방어: SOXS 스위칭을 -0.8%로 최우선 가동"
        }
    ]
    ...
    def run_backtest_simulation(self, historical_data: Dict[str, pd.DataFrame] = None) -> Dict[str, Any]:
        ...
        if historical_data is None:
            soxl_df = yf.Ticker("SOXL").history(period=self.lookback_period)
            soxs_df = yf.Ticker("SOXS").history(period=self.lookback_period)
            vix_df = yf.Ticker("^VIX").history(period=self.lookback_period)
        else:
            soxl_df = historical_data.get("SOXL")
            soxs_df = historical_data.get("SOXS")
            vix_df = historical_data.get("VIX")

        # 공통 일자 인덱스 정렬
        common_index = soxl_df.index.intersection(soxs_df.index).intersection(vix_df.index)
        soxl_close = soxl_df.loc[common_index, 'Close']
        soxs_close = soxs_df.loc[common_index, 'Close']
        vix_close = vix_df.loc[common_index, 'Close']
        
        candidates_results = []

        for candidate in self.CANDIDATE_POOL:
            res = self._simulate_single_logic(candidate, soxl_close, soxs_close, vix_close)
    ...
    def _simulate_single_logic(self, logic_cfg: Dict[str, Any], soxl: pd.Series, soxs: pd.Series, vix: pd.Series) -> Dict[str, Any]:
        ...
        soxl_ret = soxl.pct_change().fillna(0).values
        soxs_ret = soxs.pct_change().fillna(0).values
        vix_vals = vix.values

        trades = []
        equity_curve = [1.0]
        
        in_pos = "NONE" # "SOXL", "SOXS", "NONE"
        entry_price_soxl = 0.0
        entry_price_soxs = 0.0
        current_equity = 1.0

        for i in range(1, len(soxl_ret)):
            day_ret_soxl = soxl_ret[i]
            day_ret_soxs = soxs_ret[i]
            
            # 포지션이 없는 경우
            if in_pos == "NONE":
                # SOXL 눌림목 진입 조건
                if day_ret_soxl <= dip_buy and vix_vals[i] < 28.0:
                    in_pos = "SOXL"
                    entry_price_soxl = soxl.iloc[i]
                elif day_ret_soxl <= hedge_switch or vix_vals[i] >= 28.0:
                    in_pos = "SOXS"
                    entry_price_soxs = soxs.iloc[i]
            
            # SOXL 보유 중
            elif in_pos == "SOXL":
                current_pnl = day_ret_soxl
                # 칼손절 및 헤지 스위칭 검사
                if current_pnl <= stop_loss or current_pnl <= hedge_switch:
                    # 손절 후 즉시 SOXS 헤지 스위칭
                    current_equity *= (1.0 + current_pnl)
                    trades.append(current_pnl)
                    in_pos = "SOXS" if current_pnl <= hedge_switch else "NONE"
