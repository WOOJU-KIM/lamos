...
class BlackSwanKillSwitch:
    ...
    """
    def __init__(self, vix_spike_limit: float = 15.0, soxl_drop_limit: float = -7.0, vix_absolute_extreme: float = 35.0):
        self.vix_spike_limit = vix_spike_limit
        self.soxl_drop_limit = soxl_drop_limit
        self.vix_absolute_extreme = vix_absolute_extreme

    def evaluate(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        """실시간 시세를 스캔하여 블랙스완 킬스위치 발동 여부 검사"""
        vix = market_data.get("vix", {})
        soxl = market_data.get("assets", {}).get("SOXL", {})
        
        vix_price = float(vix.get("current_price", 0.0))
        vix_change_pct = float(vix.get("change_pct", 0.0))
        soxl_change_pct = float(soxl.get("change_pct", 0.0))
        ...
        # 3. SOXL 당일 -7% 이상 급락
        if soxl_change_pct <= self.soxl_drop_limit:
            triggered = True
            reasons.append(f"SOXL 3x 레버리지 당일 폭락 감지 ({soxl_change_pct:+.2f}% <= {self.soxl_drop_limit}%)")
        ...
        if triggered:
            return {
                ...
                "metrics": {
                    ...
                    "soxl_change_pct": soxl_change_pct
                }
            }
