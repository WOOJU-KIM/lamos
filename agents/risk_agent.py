...
class RiskAgent:
    ...
    def assess_risk_and_allocation(self, market_data: Dict[str, Any], strategy_decision: Dict[str, Any], regime_data: Dict[str, Any], kill_switch_status: Dict[str, Any]) -> Dict[str, Any]:
        ...
        soxl_mult = regime_params.get("soxl_weight_mult", 1.0)
        target_asset = strategy_decision.get("target_asset", "SOXL")
...
[리스크 심사 원칙]
1. VIX < 18 (저변동성): 기준 비중 80~100%에 국면 가중치({soxl_mult}x) 적용.
2. VIX 18 ~ 25 (중변동성): 기준 비중 50~60%로 제한.
3. VIX > 25 (고변동성): 기준 비중 20~30%로 축소.
4. SOXS 헷지 포지션 진입 시에는 약세 국면 가중치 적용.
...
- 시장 국면: {regime_data.get('regime_name_kr')} (가중치 {soxl_mult}x)
- SOXL 현재가: ${market_data.get('assets', {}).get('SOXL', {}).get('current_price')} ({market_data.get('assets', {}).get('SOXL', {}).get('change_pct')}%)
...
위 제안을 심사하고 최종 진입 비중(%)과 승인 결정을 JSON으로 작성하세요."""
        ...
        try:
            ...
            alloc = int(min(100, max(10, 80 * soxl_mult))) if vix_price < 18 else 50
            parsed = {
                "risk_level": "LOW" if vix_price < 18 else "MODERATE",
                "vix_analysis": f"VIX {vix_price}pt 기준 안정적 변동성 유지 중",
                "approved_action": strategy_decision.get("action", "BUY_SOXL_DIP"),
